...
def compose(
    ...
    *,
    ...
    # ---- trips + stop_times ------------------------------------------------
    # Group STOP_TIME claims by trip, ordered by declared sequence.
    ...
    for c in cs.of_kind(ClaimKind.STOP_TIME):
        ...
        by_trip.setdefault(tkey, []).append(c)
    ...
    for tkey in sorted(by_trip):
        cells = sorted(by_trip[tkey], key=lambda x: int(x.scope.get("seq", 0)))
        meta = trip_meta.get(tkey)
        rkey = str((meta.scope.get("route") if meta else None)
                   or cells[0].scope.get("route") or "")
        skey = str((meta.scope.get("service") if meta else None)
                   or cells[0].scope.get("service") or "")
        ...
        raw_times = [None if c.is_illegible else parse_hhmm(c.value) for c in cells]
        times = normalise_trip_times(raw_times)

        served = [(c, t) for c, t in zip(cells, times) if t is not None]
        if len(served) < 2:
        ...
        tables["trips.txt"].append({
            ...
            "trip_headsign": (meta.value if meta and meta.field == "trip_headsign"
                              else cells[-1].scope.get("headsign", "")),
            "direction_id": (meta.scope.get("direction_id", 0) if meta else 0),
            "wheelchair_accessible": (meta.scope.get("wheelchair_accessible", 0)
                                      if meta else 0),
        })

        for seq, (c, t) in enumerate(served, start=1):
            skey_stop = str(c.scope.get("stop") or "")
            if skey_stop not in stop_ids:
                raise ComposeError(
                    f"stop_time {c.claim_id} references unknown stop {skey_stop!r}")
            ...
            boardable = bool(c.scope.get("boardable", True))
    ...
    tables["feed_info.txt"].append({
        ...
        "feed_version": feed_version or cs.sha256()[:12],
        "feed_contact_email": "",
    })
    ...
    if ungeocoded and require_coordinates:
